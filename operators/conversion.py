"""
Operadores de conversión corregidos - sin aplicación redundante de pose
"""
...
class UNIVERSALGTA_OT_execute_conversion(Operator):
    """Ejecuta la conversión a GTA SA"""
    bl_idname = "universalgta.execute_conversion"
    bl_label = "Convert to GTA SA"
    bl_description = "Convierte el modelo de universal a formato GTA SA"
    ...
    def validate_conversion_setup(self, settings):
        ...
        if not settings.source_armature:
            return {'valid': False, 'message': "No se ha seleccionado armature fuente"}
        
        if not settings.target_armature:
            return {'valid': False, 'message': "No se ha seleccionado armature destino"}
        
        if settings.source_armature == settings.target_armature:
            return {'valid': False, 'message': "Los armatures fuente y destino no pueden ser el mismo"}
        
        if len(settings.bone_mappings) == 0:
            return {'valid': False, 'message': "No hay mapeos de huesos configurados"}
        
        enabled_mappings = [m for m in settings.bone_mappings if m.enabled]
        if len(enabled_mappings) == 0:
            return {'valid': False, 'message': "No hay mapeos de huesos habilitados"}
    ...
    def execute(self, context):
        ...
        try:
            ...
            # Ejecutar conversión (la pose personalizada se aplica dentro del convert())
